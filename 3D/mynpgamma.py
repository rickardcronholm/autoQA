...
def _calculate_min_dose_difference(
    ...
    if kwargs['local_dose']:
		relative_dose_diff = np.array([
		    dose_evaluation[to_be_checked] / reference_interpolation(points)
		    for points in all_points
		])
		dose_difference = 1 - relative_dose_diff
    else:
...
def _calculation_loop(**kwargs):
    ...
    while True:
        ...
        dose_thres = np.where(to_be_checked, kwargs['dose_threshold'], 
            np.nan)
        dose_thres = dose_thres[~np.isnan(dose_thres)]

        gamma_at_distance = np.sqrt(
            min_dose_difference ** 2 / dose_thres ** 2 +
...
def calc_gamma(coords_reference, dose_reference,
    ...
    Args:
        ...
        local_dose (:obj:`bool`, optional): Switch between local and global 
            dose deviation in the dose check.
    ...
    #dose_threshold = np.ravel(dose_threshold)
